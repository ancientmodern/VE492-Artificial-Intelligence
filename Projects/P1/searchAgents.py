...
class Graph:
    ...
    def nearest(self, start):
        ...
        while len(visited) < len(self.V):
            prevV = nextV
            minD = 99999
            for v in (set(self.V) - set(visited)):
                dist = util.manhattanDistance(prevV, v)
                if dist < minD:
                    minD = dist
                    nextV = v
...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        self.corners = {0: (1, 1), 1: (1, top), 2: (right, 1), 3: (right, top)}
        for index in self.corners:
            if not startingGameState.hasFood(*self.corners[index]):
                print('Warning: no food in corner ' + str(self.corners[index]))
            ...
            if not hitsWall:
                ...
                for corner in state[1].values():
                    if corner != (nextx, nexty):
                        nextCorners[i] = corner
                        i += 1
                nextState = (nextPosition, nextCorners)
                successors.append((action, 1, nextState))
...
def cornersHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    # my idea: maximum of manhattan distances to remaining corners
    if not state[1]:
        return 0
    x, y = state[0]
    return max([abs(x - corner[0]) + abs(y - corner[1]) for corner in state[1].values()])
    ...
    def getSuccessors(self, state):
        ...
        for direction in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextx][nexty]:
                ...
                successors.append((direction, 1, ((nextx, nexty), nextFood)))
        return successors
...
def foodHeuristic(state, problem):
    ...
    for food in foodGrid.asList():
        ...
        maxD = max(maxD, len(search.bfs(prob)))
    return maxD
    '''
    ...
    # g.addEdge(position, minNode, minD)
    g.addEdge(position, minNode, mazeDistance(position, minNode, problem.startingGameState))
    return g.KruskalMST()
    ...
    g = Graph(nodes)
    return g.nearest(position)
    '''
    # distances = [mazeDistance(position, food, problem.startingGameState) for food in foodGrid.asList()]
    # return mazeDistance(position, (maxX, maxY), problem.startingGameState)
