...
def consistent(assignment, csp, var, value):
    ...
    for c in csp.binaryConstraints:
        if c.affects(var):
            otherVar = c.otherVariable(var)
            if assignment.isAssigned(otherVar):
                if not c.isSatisfied(value, assignment.assignedValues[otherVar]):
                    return False
    return True
...
def recursiveBacktracking(assignment, csp, orderValuesMethod, selectVariableMethod, inferenceMethod):
    ...
    for value in orderValuesMethod(assignment, csp, var):
        if consistent(assignment, csp, var, value):
            ...
            # if inferences is None, then it means this assigned value fails, go to backtrack
            if inferences is not None:
                result = recursiveBacktracking(assignment, csp, orderValuesMethod, selectVariableMethod,
                                               inferenceMethod)
                if result is not None:
                    return result
            # if some result is wrong, then it means this assigned value fails, go to backtrack
            # remember this time if inference is not empty, we also need to reassign them
            # the next 4 lines are all used for reversing
            if inferences:
...
def minimumRemainingValuesHeuristic(assignment, csp):
    ...
    for var in domains:
        if not assignment.isAssigned(var):
            if len(domains[var]) < minD:
                minD = len(domains[var])
                nextVar = var
    return nextVar
...
def leastConstrainingValuesHeuristic(assignment, csp, var):
    ...
    """
    # TODO: Question 3
    cvs = list()  # a list contain (count, value) pair
    for value in assignment.varDomains[var]:
        ...
        for constraint in (c for c in csp.binaryConstraints if c.affects(var)):
            ...
            for otherValue in assignment.varDomains[otherVar]:  # remaining values for other variable before assignment
                if not constraint.isSatisfied(value, otherValue):
                    count += 1
        cvs.append((count, value))
    cvs.sort()  # sort by count
    return [cv[1] for cv in cvs]  # list of value
...
def forwardChecking(assignment, csp, var, value):
    ...
    """
    inferences = set([])
    # inferences is a list storing the removed (variable, value) pairs
    # TODO: Question 4
    for constraint in (c for c in csp.binaryConstraints if c.affects(var)):
        otherVar = constraint.otherVariable(var)
...
def revise(assignment, csp, var1, var2, constraint):
    ...
    # TODO: Question 5
    # This "revise" explicitly specify the constraint, different to the pseudo code
...
def maintainArcConsistency(assignment, csp, var, value):
    ...
    # maintainArcConsistency is an updated version of forward checking, but it is still
    # limited in one variable, only cares about the variable that has been assigned recently
    queue = deque()
    ...
    for constraint in (c for c in csp.binaryConstraints if c.affects(var)):
        ...
        if not assignment.isAssigned(otherVar):
            # "revise" specify the constraint, so here we also need to contain it in the queue
            queue.append((var, otherVar, constraint))

    # when queue is not empty
    while len(queue) > 0:
        Xi, Xj, constraint = queue.pop()
        # temp is inference used for this constraint or None (means inconsistent)
        temp = revise(assignment, csp, Xi, Xj, constraint)
        if temp is None:
            # if inconsistent, backtrack, and also return None (means inconsistent)
            for inference in inferences:
                domains[inference[0]].add(inference[1])
            return None
        if len(temp):
            # if not empty (means something has been deleted), first union temp into the
            # whole inferences (used for this assignment), then deal with the neighbours of Xj
            inferences = inferences.union(temp)
            for neighbour in (c for c in csp.binaryConstraints if c.affects(Xj)):
                otherVar = neighbour.otherVariable(Xj)
                if not assignment.isAssigned(otherVar):
                    # if neighbour is not assigned, add it to queue again
                    queue.append((Xj, otherVar, neighbour))
    return inferences
...
def AC3(assignment, csp):
    ...
    # the only difference between AC3 and maintainArcConsistency is the initialization part,
    # where AC3 consider all variables
    for var in csp.varDomains:
        for constraint in (c for c in csp.binaryConstraints if c.affects(var)):
